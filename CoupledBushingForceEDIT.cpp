/* -------------------------------------------------------------------------- *
*                     OpenSim:  CoupledBushingForceEDIT.cpp                      *
* -------------------------------------------------------------------------- *
...
* Destructor.
*/
CoupledBushingForceEDIT::~CoupledBushingForceEDIT()
{
}
//_____________________________________________________________________________
/**
* Default constructor.
*/
CoupledBushingForceEDIT::CoupledBushingForceEDIT() : Force(),
	...
	_orientationInBody2(_orientationInBody2Prop.getValueDblVec())
{
	...
	setupStiffnessFunctions();
	setNull();
}

/* Convenience constructor */
CoupledBushingForceEDIT::CoupledBushingForceEDIT( std::string body1Name, SimTK::Vec3 point1, SimTK::Vec3 orientation1,
...
* Copy constructor.
*
* @param aForce CoupledBushingForceEDIT to be copied.
*/
CoupledBushingForceEDIT::CoupledBushingForceEDIT(const CoupledBushingForceEDIT &aForce) :
Force(aForce),
	_body1Name(_body1NameProp.getValueStr()),
	_body2Name(_body2NameProp.getValueStr()),
	_locationInBody1(_locationInBody1Prop.getValueDblVec()),
...
//_____________________________________________________________________________
/**
* Copy data members from one CoupledBushingForceEDIT to another.
*
* @param aForce CoupledBushingForceEDIT to be copied.
*/
void CoupledBushingForceEDIT::copyData(const CoupledBushingForceEDIT &aForce)
{
...
//_____________________________________________________________________________
/**
* Set the data members of this CoupledBushingForceEDIT to their null values.
*/
void CoupledBushingForceEDIT::setNull()
{
...
*/
void CoupledBushingForceEDIT::setupProperties()
{
	// Body 1 name
...
*
* @param aModel OpenSim model containing this CoupledBushingForceEDIT.
*/
void CoupledBushingForceEDIT::connectToModel(Model& aModel)
{
...
void CoupledBushingForceEDIT::addToSystem(SimTK::MultibodySystem& system) const
{
	...
	CoupledBushingForceEDIT* mutableThis = const_cast<CoupledBushingForceEDIT *>(this);
...
*/
CoupledBushingForceEDIT& CoupledBushingForceEDIT::operator=(const CoupledBushingForceEDIT &aForce)
{
...
void CoupledBushingForceEDIT::setBody1ByName(std::string aBodyName)
{
	_body1Name = aBodyName;
}

void CoupledBushingForceEDIT::setBody2ByName(std::string aBodyName)
{
	_body2Name = aBodyName;
}

/** Set the location and orientation (optional) for weld on body 1*/
void CoupledBushingForceEDIT::setBody1BushingLocation(Vec3 location, Vec3 orientation)
{
...
void CoupledBushingForceEDIT::setBody2BushingLocation(Vec3 location, Vec3 orientation)
{
...
void CoupledBushingForceEDIT::setStiffness(Mat66 K)
{
...
void CoupledBushingForceEDIT::setDamping(Mat66 D)
{
...
void CoupledBushingForceEDIT::setF33(const OpenSim::Function& f1)
{
	set_m_function33( f1 );
}

void CoupledBushingForceEDIT::setF34(const OpenSim::Function& f1)
{
	set_f_function34( f1 );
}

void CoupledBushingForceEDIT::setF35(const OpenSim::Function& f1)
{
	set_f_function35( f1 );
}

void CoupledBushingForceEDIT::setF43(const OpenSim::Function& f1)
{
	set_m_function43( f1 );
}

void CoupledBushingForceEDIT::setF44(const OpenSim::Function& f1)
{
	set_f_function44( f1 );
}

void CoupledBushingForceEDIT::setF45(const OpenSim::Function& f1)
{
	set_f_function45( f1 );
}

void CoupledBushingForceEDIT::setF53(const OpenSim::Function& f1)
{
	set_m_function53( f1 );
}

void CoupledBushingForceEDIT::setF54(const OpenSim::Function& f1)
{
	set_f_function54( f1 );
}

void CoupledBushingForceEDIT::setF55(const OpenSim::Function& f1)
{
	set_f_function55( f1 );
}
...
SimTK::Vec6 CoupledBushingForceEDIT::computeDeflection(const SimTK::State& s) const
{
...
* CoupledBushingForceEDIT implementation based SimTK::Force::LinearBushing
* developed and implemented by Michael Sherman.
*/
void CoupledBushingForceEDIT::computeForce(const SimTK::State& s, 
	...
	SimTK::Vector& generalizedForces) const
{
	...
	Vec6 fk(0.0);
	//fk = _stiffnessMatrix*dq;

	//%%%%%%%%%%%%%%%%%%%%%%%%%%%//
	// calculate stiffness forces using stiffness functions...
	Vec6 fk_edit(0.0);

	// f1 = f(theta1) + f(theta2) + f(theta3) + f(x) + f(y) + f(z)
	// define...
	// f1 = F11(dq[0]) + F12(dq[1]) + F13(dq[2]) + F14(dq[3]) + F15(dq[4]) + F16(dq[5])
	// f2 = F21(dq[0]) + F22(dq[1]) + F23(dq[2]) + F24(dq[3]) + F25(dq[4]) + F26(dq[5])
	// NOTE: Torque = f(theta) + ***f(x) + f(y)*** -> do I need to take into account the units??
	...
	//
	//// Need to add additional terms to fk[i] which include additional functions multiplying the
	//// cross terms of dq...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%//
...
double CoupledBushingForceEDIT::computePotentialEnergy(const SimTK::State& s) const
{
	Vec6 dq = computeDeflection(s);
...
* 
*/
OpenSim::Array<std::string> CoupledBushingForceEDIT::getRecordLabels() const 
{
...
*/
OpenSim::Array<double> CoupledBushingForceEDIT::getRecordValues(const SimTK::State& state) const 
{
...
/* UTILITIES */
void CoupledBushingForceEDIT::constructMatricesFromProperties()
{
...
void CoupledBushingForceEDIT::updatePropertiesFromMatrices()
{
...
void CoupledBushingForceEDIT::setupStiffnessFunctions()
{
	//set all deflection functions to 10.0
	OpenSim::Constant ZeroFunc = Constant(0.0);
	constructProperty_m_function11( ZeroFunc );
	constructProperty_m_function22( ZeroFunc );

	constructProperty_m_function33( ZeroFunc );
	constructProperty_f_function34( ZeroFunc );
	constructProperty_f_function35( ZeroFunc );

	constructProperty_m_function43( ZeroFunc );
	constructProperty_f_function44( ZeroFunc );
	constructProperty_f_function45( ZeroFunc );

	constructProperty_m_function53( ZeroFunc );
	constructProperty_f_function54( ZeroFunc );
	constructProperty_f_function55( ZeroFunc );

	constructProperty_f_function66( ZeroFunc );

	set_m_function11(ZeroFunc);
	set_m_function22(ZeroFunc);

	set_m_function33(ZeroFunc);
	set_f_function34(ZeroFunc);
	set_f_function35(ZeroFunc);

	set_f_function44(ZeroFunc);
	set_f_function45(ZeroFunc);
	set_m_function43(ZeroFunc);

	set_f_function54(ZeroFunc);
	set_f_function55(ZeroFunc);
	set_m_function53(ZeroFunc);

	set_f_function66(ZeroFunc);
